...
BMI08X_INTF_RET_TYPE bmi08x_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
	...
	uint8_t res = HAL_I2C_Mem_Read(&hi2c1, (dev_addr)<<1, reg_addr, I2C_MEMADD_SIZE_8BIT, (uint8_t *)reg_data, len, 100);
	return res;
...
{
	uint8_t dev_addr = *(uint8_t*)intf_ptr;
	uint8_t res = HAL_I2C_Mem_Write(&hi2c1, (dev_addr)<<1, reg_addr, I2C_MEMADD_SIZE_8BIT, (uint8_t *)reg_data, len, 100);
	return res;
